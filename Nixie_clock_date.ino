...
#define DATE_TIME 5        // время (с), которое отображается дата
...
void modeTick() {
  if (modeTimer.isReady()) {
    if (mode == 1) {
      ...
      digitsDraw[1] = day / 10;
      digitsDraw[2] = day % 10;
      digitsDraw[3] = month / 10;
      digitsDraw[4] = month % 10;
      digitsDraw[5] = year / 10;
      digitsDraw[6] = year % 10;

      mode = 1;
      modeTimer.setInterval((long)DATE_TIME * 1000);
    }
  }
}
